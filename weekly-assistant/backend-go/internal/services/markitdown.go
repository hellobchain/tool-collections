...
// docxStyleInfo holds style metadata read from styles.xml
type docxStyleInfo struct {
	StyleID    string `xml:"styleId,attr"`
	Type       string `xml:"type,attr"`
	Name       string `xml:"name>val,attr"`
	OutlineLvl int    `xml:"pPr>outlineLvl>val,attr"`
}
...
// docxDocument for full XML parse
...
type docxPPr struct {
	StyleID       string     `xml:"pStyle>val,attr"`
	NumPr         *docxNumPr `xml:"numPr"`
	Justification string     `xml:"jc>val,attr"`
}

type docxNumPr struct {
	Ilvl  string `xml:"ilvl>val,attr"`
	NumID string `xml:"numId>val,attr"`
}
...
type docxRPr struct {
	Bold      *struct{} `xml:"b"`
	BoldVal   string    `xml:"b>val,attr"`
	Italic    *struct{} `xml:"i"`
	ItalicVal string    `xml:"i>val,attr"`
	Underline *struct{} `xml:"u"`
	Color     string    `xml:"color>val,attr"`
	RFonts    string    `xml:"rFonts>ascii,attr"`
	Sz        string    `xml:"sz>val,attr"`
}
...
type docxLvl struct {
	...
	NumFmt  string `xml:"numFmt>val,attr"`
	LvlText string `xml:"lvlText>val,attr"`
}

type docxNum struct {
	NumID         string `xml:"numId,attr"`
	AbstractNumID string `xml:"abstractNumId>val,attr"`
}
...
func (m *Markitdown) parseDocxStyles(data []byte) map[string]int {
	...
	// Map style IDs that have outlineLvl set (headings)
...
func (m *Markitdown) parseDocxNumbering(data []byte) map[string]string {
	...
	// Build abstractNum ID -> numFmt mapping
...
func (m *Markitdown) writeDocxBody(md *strings.Builder, doc docxDocument, headingStyles map[string]int, rels map[string]string, numFmts map[string]string) {
	...
	// Sort elements by their position (they come in document order already)
...
func (m *Markitdown) writeDocxTable(md *strings.Builder, table docxTable, headingStyles map[string]int, rels map[string]string, numFmts map[string]string) {
	if len(table.Rows) == 0 {
		return
	}
	...
	m.writeTableRow(md, table.Rows[0], maxCols, headingStyles, rels, numFmts)
	// Write separator
	md.WriteString("|" + strings.Repeat(" --- |", maxCols) + "\n")
	// Write remaining rows
	for i := 1; i < len(table.Rows); i++ {
		m.writeTableRow(md, table.Rows[i], maxCols, headingStyles, rels, numFmts)
	}
	md.WriteString("\n")
}

func (m *Markitdown) writeTableRow(md *strings.Builder, row docxTableRow, maxCols int, headingStyles map[string]int, rels map[string]string, numFmts map[string]string) {
	md.WriteString("|")
...
func (m *Markitdown) convertDoc(data []byte) (*Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse OLE2: %w", err)
	}
	...
	for entry, err := doc.Next(); err == nil; entry, err = doc.Next() {
		if entry.Name == "WordDocument" {
			...
			if _, err := io.Copy(buf, entry); err != nil {
				return nil, fmt.Errorf("failed to read WordDocument: %w", err)
			}
	...
	if text == "" {
		text = extractPrintableStrings(wordStream)
	}
